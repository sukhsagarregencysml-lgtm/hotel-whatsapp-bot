...
const { getTally, updateTally, useFreeRooms, getAllAgents, addAgent, removeAgent, listAgents } = require("./agents");
...
async function handleIncoming({ from, text, msgId, msgType, mediaId, buttonId }) {
  ...
  if (!agent || agent.category === "Guest") {
    // Handle payment screenshot from guests BEFORE routing to guest flow
  ...
  if (t === "CANCEL" || t === "CANCEL BOOKING" || t === "CANCELLATION") {
    ...
    if (agentBookings.length === 0) {
      await sendMessage(from, `❌ No active bookings found.\n\nFor confirmed bookings contact hotel:\n📞 +91 98160 03322`);
      return;
    }
    if (agentBookings.length === 1) {
      ...
      const lines = agentBookings.map((b,i)=>`*${i+1}.* ${b.voucherNo} — ${b.guestName} — ${fmtDate(b.ciDate)}`);
  ...
  if (session.step === "awaiting_cancel_select") {
    ...
    if (isNaN(idx)||idx<0||idx>=(session.cancelOptions||[]).length) { await sendMessage(from,`Please reply 1 to ${(session.cancelOptions||[]).length}`); return; }
  ...
  if (session.step === "awaiting_cancel_confirm") {
    if (["YES","Y","HAAN","HA"].includes(t)) {
      ...
      const booking = pendingPayments[from];
      if (!booking) { await sendMessage(from, `Booking not found. Contact hotel: 📞 +91 98160 03322`); session.step="idle"; return; }
      ...
      await sendMessage(from, `Cancellation cancelled. Booking still active. 🙏`);
      session.step="idle"; delete session.cancelVoucherNo; delete session.cancelOptions;
    }
    return;
  }
  ...
  // Step: awaiting extra bed — accepts CWB, CNB, numbers, text
...
async function checkAndRespond(from, agent, session) {
  try {
    ...
    if (result.available) {
      ...
      // Get cumulative tally — skip for guest bookings
        ...
        if (msUntilReminder > 0) {
          setTimeout(async () => {
            try {
              ...
              const qrUrl2 = `https://api.qrserver.com/v1/create-qr-code/?size=1000x1000&ecc=H&margin=2&data=upi://pay?pa=${UPI_ID}%26pn=Hotel%20Sukhsagar%20Regency%26am=${secondAmt}%26cu=INR%26tn=2nd-${voucherNo}`;
              await axios.post(
                `https://graph.facebook.com/v25.0/${process.env.WA_PHONE_NUMBER_ID}/messages`,
                {
                  messaging_product: "whatsapp",
...
async function handleAdminReply(from, text, t) {
  ...
  if (t.startsWith("APPROVE PAY")) {
    ...
    if (remaining <= 0) {
      ...
      // Send guest voucher (no price) + location
      try {
        ...
        await axios.post(
          ...
          { messaging_product:"whatsapp", recipient_type:"individual", to:agentPhone, type:"location",
            location:{ longitude:77.140404, latitude:31.078199, name:"Hotel Sukhsagar Regency", address:"Tara Devi, Shimla, HP 171010" }},
          { headers:{ Authorization:`Bearer ${process.env.WA_ACCESS_TOKEN}`, "Content-Type":"application/json" }}
        );
      } catch(vErr) { console.error("Guest voucher error:", vErr.message); }
      ...
      if (nextStep) {
        ...
        // Short notice — just remind about balance at check-in (no extra QR)
  ...
  if (t.startsWith("PAY RECEIVED") || t.startsWith("PAYMENT RECEIVED") || t.startsWith("PAY REC")) {
    const parts = text.trim().split(/\s+/);
    const agentPhone = parts.find((p,i) => i>0 && /^91\d{10}$/.test(p.replace(/\D/g,"")))?.replace(/\D/g,"") || null;
    ...
    const pending = pendingPayments[agentPhone];
    if (!pending) { await sendMessage(from, `✅ Payment of Rs.${amount.toLocaleString()} noted for ${agentPhone}. No active booking found.`); return; }
    ...
    await sendReminder(agentPhone,
      ...
      (remaining<=0 ? `🎉 All payments complete!` : `💡 Rs.${remaining.toLocaleString()} due at check-in.`) + `\n\nThank you! 🙏`
    );
  ...
  if (t === "LIST CANCEL") {
    ...
    const lines = keys.map(k => { const c=pendingCancellations[k]; return `• ${c.voucherNo} — ${c.guestName} — ${fmtDate(c.ciDate)} (${c.agentName})`; });
...
` +
  ...
  // BOOK1/BOOK2/BOOK — admin booking
  ...
  if (isBook) {
    ...
    try {
      ...
      const fakeSession={ciDate:parsed.ciDate,coDate:parsed.coDate,roomType,rooms,roomTypes:roomTypes.length>1?roomTypes:null,plan:plan.toUpperCase(),rate:finalRate,grandTotal:Math.round(grandTotal),nights,adults:parsed.adults||rooms*2,kids:0,guestName,guestMobile:fullPhone,extraBed:0,extraBedCharge:0,extraBedType:null,remark,isGuestBooking:isBook1};
      await sendMessage(from,`⏳ Creating for *${guestName}*...\n${fmtDate(parsed.ciDate)} → ${fmtDate(parsed.coDate)}\n${roomSummary} | ${plan}\nTotal: *Rs.${Math.round(grandTotal).toLocaleString()}* ${rateTypeLabel}${remarkText}`);
      await confirmAndSave(fullPhone,agent2,fakeSession);
      await sendMessage(from,`✅ *Booking created!*\n\nGuest: ${guestName} (${fullPhone})\nCheck-in: ${fmtDate(parsed.ciDate)}\nCheck-out: ${fmtDate(parsed.coDate)}\nRooms: ${roomSummary}\nPlan: ${plan}\nTotal: *Rs.${Math.round(grandTotal).toLocaleString()}*\nAdvance (20%): Rs.${advanceAmount.toLocaleString()}${remarkText}\n\nVoucher + QR sent ✅`);
